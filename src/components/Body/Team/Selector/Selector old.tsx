...
export default function Selector({
  ...
  const selectedTeams = teamsSelectedIds.filter(
    (team: string) => team !== teamId,
  );
  let selectableTeams = activeTeams.filter(
    (team: TeamType) => !selectedTeams.includes(team.id),
  );
  ...
  const changeTeam = async (event: { value: string }) => {
    ...
    const newAvailableTeams = activeTeams.filter(
      (team: TeamType) => !selectedTeams.includes(team.id),
    );
